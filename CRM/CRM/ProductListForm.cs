...
namespace CRM
{
    public partial class ProductListForm : Form
    {
        ...
        {
            decimal topla = 0;
            foreach (DataRow item in shoppingcart.Rows)
            {
                topla += Convert.ToDecimal(item["TotalPrice"]);
            }
            label1.Text = "Total:" + topla;
        }
        private void ShoppingCartDataGrid_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (shoppingcart.Rows.Count > 0)
            {
                shoppingcart.Rows.RemoveAt(e.RowIndex);
                ShoppingCartDataGrid.DataSource = shoppingcart;
                toplagoster();
            }
        }
